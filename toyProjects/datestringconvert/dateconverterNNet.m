...
rng(RNGseed)
    ...
    Yeari = randi([2014,2020],[Nobservations,1]);
    Monthi = randi([1,12],[Nobservations,1]);
    Dayi = randi([1,31],[Nobservations,1]);
    InputDateStr = repmat( string(), Nobservations , 1);
    
    for n=1:Nobservations
        switch    randi(3)
            case 1
%                 InputDateStr(n,:) = string(datestr(datetime(Yeari(n),Monthi(n),Dayi(n)),'dd dddd mmm yy'));
                InputDateStr(n,:) = string(datestr(datetime(Yeari(n),Monthi(n),Dayi(n)),'yy mmmm dddd dd'));
%                 InputDateStr(n,:) = string(datestr(datetime(Yeari(n),Monthi(n),Dayi(n)),'ddmm yyyy'));
%                 InputDateStr(n,:) = string(datestr(datetime(Yeari(n),Monthi(n),Dayi(n)),'yyyy mmm'));
            case 2
                InputDateStr(n,:) = string(datestr(datetime(Yeari(n),Monthi(n),Dayi(n)),'dddd-dd-mmm-yy'));
%                 InputDateStr(n,:) = string(datestr(datetime(Yeari(n),Monthi(n),Dayi(n)),'dd-mmm-yy'));
%                 InputDateStr(n,:) = string(datestr(datetime(Yeari(n),Monthi(n),Dayi(n)),'mmm-yyyy'));
            case 3
                InputDateStr(n,:) = strcat(string(datestr(datetime(Yeari(n),Monthi(n),Dayi(n)),'yyyy mm dddd dd')));
%                 InputDateStr(n,:) = strcat(string(datestr(datetime(Yeari(n),Monthi(n),Dayi(n)),'yy dd mmm')));
%                 InputDateStr(n,:) = string(datestr(datetime(Yeari(n),Monthi(n),Dayi(n)),'yyyy mmm'));
            case 4
%                 InputDateStr(n,:) = strcat(string(datestr(datetime(Yeari(n),Monthi(n),Dayi(n)),'mmm-dddd-dd-yy')));
%                 InputDateStr(n,:) = strcat(string(datestr(datetime(Yeari(n),Monthi(n),Dayi(n)),'mmm-dd-yy')));
%                 InputDateStr(n,:) = string(datestr(datetime(Yeari(n),Monthi(n),Dayi(n)),'mmm-yyyy'));
        end
    end
    
    OutputDateStr = string( strcat(num2str([Yeari]),'-',num2str([Monthi]),'-',num2str([Dayi])) );
%     OutputDateStr = string( strcat(num2str([Yeari]),'-',num2str([Monthi])) );
    
    Data(:,1) = cellstr(InputDateStr);
    Data(:,2) = cellstr(OutputDateStr);

% get list of all potential tokens in the datasets
AllKey = unique([string(unique(char(OutputDateStr(:))))' , string(unique(char(InputDateStr(:))))']);
Ykey = AllKey ;
Xkey = AllKey ;
